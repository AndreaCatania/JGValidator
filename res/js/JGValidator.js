...
var JGValidator = function( $elements, isLive ){
	...
	/**
	 * This method replace all current inputs to check
	 */
	...
	self.addElements = function( $elementsToAdd ){
		$elementsToAdd.each(function(){
			self.addElement($(this));
		});
	};

	self.addElement = function( $elementToAdd ){
		$elements = $elements.add($elementToAdd);
		self.loadEventHand($elementToAdd);
	};

	self.removeElements = function( $elementsToRemove ){
		$elementsToRemove.off("change input blur");
		$elements = $elements.not($elementsToRemove);
	};
	...
	/**
	 * If isLive is true the inputs are checked in real time
	 */
	self.loadAllEventHand = function(){
		if( isLive ){
			self.loadEventHand($( this ));
		}
	};

	self.loadEventHand = function($inp){
		if( isLive ){
			var event = "";
			 ...
			 */
			$inp.on( self.eventForClear, function() {
